...
//         require(
//             seller == msg.sender || buyer == msg.sender,
...
//         require(
//             status == OrderStatus.BLANK,
//             "Cant create with the current status"
//         );
...
//         require(
//             status == OrderStatus.CREATED,
//             "Cant Initiate with the current status"
//         );
...
//         emit OrderReceived();
//         // add Aave logic
//         lendingPool.withdraw(token, type(uint256).max, seller);
//     }
...
//         require(
//             sendBlock + numBlocksToExpire < block.number,
...
//         if (refundBuyer) {
//             lendingPool.withdraw(token, type(uint256).max, buyer);
//         } else {
//             lendingPool.withdraw(token, type(uint256).max, seller);
//         }
//     }
